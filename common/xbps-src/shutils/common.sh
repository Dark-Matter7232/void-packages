...
setup_pkg() {
    ...
    export BUILD_CC="$XBPS_CC"
    export BUILD_CXX="$XBPS_CXX"
    export BUILD_CPP="$XBPS_CPP"
    export BUILD_FC="gfortran"
    export BUILD_LD="$XBPS_LD"
    export BUILD_CFLAGS="$XBPS_CFLAGS"
    export BUILD_CXXFLAGS="$XBPS_CXXFLAGS"
    export BUILD_CPPFLAGS="$XBPS_CPPFLAGS"
    # Check if using ld.lld and set linker option if not set
    if [ "$XBPS_LD" = "ld.lld" ] && [[ ! "$XBPS_LDFLAGS" =~ "-fuse-ld=lld" ]]; then
        XBPS_LDFLAGS+=" -fuse-ld=lld"
    fi
    export BUILD_LDFLAGS="$XBPS_LDFLAGS"
    export BUILD_FFLAGS="$XBPS_FFLAGS"

    export CC_FOR_BUILD="$XBPS_CC"
    export CXX_FOR_BUILD="$XBPS_CXX"
    export CPP_FOR_BUILD="$XBPS_CPP"
    export FC_FOR_BUILD="gfortran"
    export LD_FOR_BUILD="$XBPS_LD"
    ...
    if [ -n "$cross" ]; then
        ...
        if [ "$XBPS_COMPILER" = "clang" ]; then
	        ...
	        export AR="$XBPS_AR"
	        export AS="$XBPS_AS"
	        export RANLIB="$XBPS_RANLIB"
	        export STRIP="$XBPS_STRIP"
	        export OBJDUMP="$XBPS_OBJDUMP"
	        export OBJCOPY="$XBPS_OBJCOPY"
	        export NM="$XBPS_NM"
	        export READELF="$XBPS_READELF"
